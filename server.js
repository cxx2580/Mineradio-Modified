...
function mapSongRecord(s) {
  ...
  const originCoverType = Number(s.originCoverType);
  return {
    provider: 'netease',
    source: 'netease',
    type: 'song',
    ...
    fee: s.fee,
    alias: Array.isArray(s.alia) ? s.alia : (Array.isArray(s.alias) ? s.alias : []),
    originCoverType: Number.isFinite(originCoverType) ? originCoverType : null,
    searchScore: Number(s.score || s.searchScore || s.algScore || 0) || 0,
    popularity: Number(s.pop || s.popularity || s.playCount || s.playcount || 0) || 0,
  };
}
...
function cleanSearchTitle(value) {
  return normalizeSearchText(value)
    .replace(/\s*[\(\（\[][^\)\）\]]*(?:feat\.?|featuring|with|live|cover|remix|dj|伴奏|翻唱|翻自|现场|片段|剪辑|抖音|加速|降调|升调|男声|女声|烟嗓|纯音乐|instrumental|karaoke|slowed|reverb|nightcore)[^\)\）\]]*[\)\）\]]\s*/ig, ' ')
    .replace(/\s*[-–—_]\s*(?:live|cover|remix|dj|伴奏|翻唱|翻自|现场|片段|剪辑|抖音|加速|降调|升调|男声|女声|烟嗓|纯音乐|instrumental|karaoke|slowed|reverb|nightcore).*/ig, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
function searchTokens(value) {
  return normalizeSearchText(value)
    .split(/[^a-z0-9\u3400-\u9fff]+/i)
    .map(t => t.trim())
    .filter(t => t && (t.length > 1 || /[\u3400-\u9fff]/.test(t)));
}

const ORIGINAL_ARTIST_HINTS = {
  [compactSearchText('日落大道')]: ['梁博'],
  [compactSearchText('Beauty and a Beat')]: ['Justin Bieber', '贾斯汀比伯', '贾斯汀·比伯'],
};

function originalArtistHintScore(song, compactQuery) {
  const hints = ORIGINAL_ARTIST_HINTS[compactQuery];
  if (!hints || !hints.length) return 0;
  const artistText = compactSearchText([
    song && song.artist,
    ...(Array.isArray(song && song.artists) ? song.artists.map(a => a && a.name) : []),
  ].filter(Boolean).join(' '));
  if (!artistText) return 0;
  let score = 0;
  hints.forEach((name, index) => {
    if (artistText.includes(compactSearchText(name))) score += index === 0 ? 420 : 120;
  });
  return score;
}

function searchVariantPenalty(song, queryText) {
  const wantsVariant = /(live|cover|remix|dj|伴奏|翻唱|现场|片段|抖音|instrumental|karaoke|slowed|reverb|nightcore)/i.test(queryText);
  if (wantsVariant) return 0;
  const text = normalizeSearchText([
    song && song.name,
    song && song.album,
    song && song.artist,
    ...(Array.isArray(song && song.alias) ? song.alias : []),
  ].filter(Boolean).join(' '));
  let penalty = 0;
  if (/(翻唱|翻自|cover|covered by|remake)/i.test(text)) penalty += 260;
  if (/(伴奏|instrumental|karaoke|纯音乐)/i.test(text)) penalty += 240;
  if (/(remix|bootleg|mashup|dj|nightcore|slowed|reverb|加速|降调|升调)/i.test(text)) penalty += 180;
  if (/(live|现场|音乐节|演唱会)/i.test(text)) penalty += 120;
  if (/(片段|剪辑|抖音|铃声|手机铃声|烟嗓|男声版|女声版)/i.test(text)) penalty += 160;
  if (/(群星|various artists|unknown artist|精彩|热门歌曲)/i.test(text)) penalty += 60;
  if (/(原唱|original|album version|专辑版|official)/i.test(text)) penalty -= 80;
  return penalty;
}

function searchPopularityScore(song) {
  const raw = Number(song && (song.popularity || song.searchScore || song.playCount || song.playcount || 0)) || 0;
  if (raw <= 0) return 0;
  if (raw <= 100) return raw * 0.16;
  return Math.min(42, Math.log10(raw + 1) * 8);
}

function scoreSearchSong(song, keywords, index) {
  const queryText = normalizeSearchText(keywords);
  const compactQuery = compactSearchText(queryText);
  const title = normalizeSearchText(song && song.name);
  const cleanTitle = cleanSearchTitle(title);
  const compactTitle = compactSearchText(title);
  const compactCleanTitle = compactSearchText(cleanTitle);
  ...
  let score = 0;
  if (compactQuery && compactCleanTitle === compactQuery) score += 900;
  else if (compactQuery && compactTitle === compactQuery) score += 820;
  else if (compactQuery && compactCleanTitle.startsWith(compactQuery)) score += 360;
  else if (compactQuery && compactCleanTitle.includes(compactQuery)) score += 180;
  if (tokens.length && tokens.every(t => allText.includes(t))) score += 140;
  score += originalArtistHintScore(song, compactQuery);
  const coverType = Number(song && song.originCoverType);
  if (Number.isFinite(coverType)) score += coverType === 0 ? 180 : -260;
  score -= searchVariantPenalty(song, queryText);
  if (song && song.cover) score += 8;
  if (song && Number(song.duration || 0) >= 45000) score += 10;
  score += searchPopularityScore(song);
  return score - index * 0.001;
}

function sortSearchSongs(songs, keywords) {
  return (songs || [])
    .map((song, index) => ({ song, index, score: scoreSearchSong(song, keywords, index) }))
...
async function handleSearch(keywords, limit) {
  ...
  let mapped = sortSearchSongs(songs.map(s => {
...
function mapQQSmartSong(item) {
  ...
  return {
    ...
    duration: 0,
    fee: 0,
    alias: item.subtitle ? [item.subtitle] : [],
    originCoverType: null,
    searchScore: Number(item.score || item.docid || 0) || 0,
    popularity: Number(item.hot || item.listen_num || 0) || 0,
    playable: false,
  };
}
...
function mapQQTrack(track, fallback) {
  ...
  const aliases = [track.subtitle, track.subTitle, fallback.subtitle]
    .concat(Array.isArray(track.grp) ? track.grp.map(g => g && (g.name || g.title)) : [])
    .filter(Boolean);
  return {
    provider: 'qq',
    source: 'qq',
    ...
    alias: aliases,
    originCoverType: null,
    searchScore: Number(track.score || fallback.searchScore || 0) || 0,
    popularity: Number(track.popularity || track.hot || track.listen_num || fallback.popularity || 0) || 0,
    playable: false,
  };
}
...
async function handleQQSearch(keywords, limit) {
  ...
  const seen = new Set();
  const unique = detailed.filter(song => {
    const key = song && (song.mid || song.id || (song.name + '|' + song.artist));
    if (!key || seen.has(key)) return false;
    seen.add(key);
    return !!song.name;
  });
  return sortSearchSongs(unique, kw);
}
      ...
      if (typeof playlist_track_all === 'function') {
        try {
          ...
          for (let offset = 0; ; offset += pageSize) {
            ...
            const chunk = (all.body && (all.body.songs || all.body.tracks)) || [];
            if (!chunk.length) break;
            const before = rawTracks.length;
            chunk.forEach(track => {
            ...
            if (chunk.length < pageSize || rawTracks.length === before) break;
          }
        } catch (err) {
