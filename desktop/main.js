...
async function createWindow() {
  ...
  mainWindow.once('ready-to-show', () => {
    // Clamp any stored negative zoom level from previous session (fix Ctrl+- stuck)
  ...
  // Prevent Ctrl+- / Ctrl++ from getting stuck below 100% zoom
  mainWindow.webContents.on('zoom-changed', (event, zoomDirection) => {
    const zoomLevel = mainWindow.webContents.getZoomLevel();
    if (zoomLevel < 0) mainWindow.webContents.setZoomLevel(0);
  });
